...
<Note>
See an example of a [simplified marketplace dapp](https://simple-marketplace-boilerplate.pages.dev/) that enables users to mint collectibles, sell the collectibles with the Sequence Marketplace, and make purchases with USDC on `base-sepolia` by getting a top order from the Marketplace.
...
For your project, you'll need a way to authenticate your user with their wallet.
